...
int main(int argc, char *argv[])
{
	...
	if (ppuaddr == NULL) {
		...
		if (row >= 30) {
			fprintf(stderr,
				"The coordinate for row is too big! (%d >= 30)\n",
				row);
			exit(EXIT_FAILURE);
		}
		if (col >= 32) {
			fprintf(stderr,
				"The coordinate for col is too big! (%d >= 32)\n",
				col);
			exit(EXIT_FAILURE);
		}
		...
		if (addr < 0x2000) {
			fprintf(stderr,
				"The ppu coordinate is too small! ($%04x < $2000)\n",
				addr);
			exit(EXIT_FAILURE);
		}
		if (addr > 0x23bf) {
			fprintf(stderr,
				"The ppu coordinate is too big! ($%04x >= $23c0)\n",
				addr);
			exit(EXIT_FAILURE);
		}
		col = addr % 0x20;
